...
const MenuTree = ({ changePos }) => {
  ...
  useEffect(
    (_) => {
      ...
      // console.log(
      //   statusIcn.filter((c) => c.running),
      //   "paolo"
      // );
      // console.log(
      //   stateReducer?.firebase?.Vehicles.map((t) => t.VehicleStatus),
      //   "muhammed"
      // );
    },
    [
  ...
  const onCheck = (selectedKeys, info) => {
    ...
    console.log(info, "ms14");
    if (byGroup.length > 0) {
      dispatch(SyncOnCheck(byGroup[0].node.children));
      changePos([43.653225, -79.383186]);
      console.log("1");
    } else if (info.checkedNodes.length > 0) {
      dispatch(SyncOnCheck(info?.checkedNodes));
      changePos(
        [
          info?.checkedNodes[info?.checkedNodes.length - 1].data.Latitude,
          info?.checkedNodes[info?.checkedNodes.length - 1].data.Longitude,
        ],
        info?.checkedNodes[info?.checkedNodes.length - 1].data
      );
      console.log("2");
    } else {
  ...
  const Icon = (props) => {
    ...
    if (pos.split("-").length === 2) {
      ...
      return (
        <i
          className={
            OverSpeedingStatus ? Styles.car__running : Styles.car__over_speed
          }
        />
      );
    }
